...
- **Routing rules**: Routing rules should be updated automatically to send traffic over the buckwild network when a host is added to the configuration.
...
- **Key Rotation**: Zero old keys when new keys are generated after the last session is using them.
