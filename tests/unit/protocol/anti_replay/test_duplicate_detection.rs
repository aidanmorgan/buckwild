use std::net::IpAddr;
use std::time::Duration;
use buckwild_common::protocol::{DuplicateDetector, DuplicateDetectionResult};

#[test]
fn test_unique_packet_detection() {
    let detector = DuplicateDetector::new(1000);
    
    let result = detector.detect_duplicate(
        12345,
        67890,
        1,
        Some(IpAddr::from([192, 168, 1, 1])),
    ).unwrap();

    assert_eq!(result, DuplicateDetectionResult::Unique);
}

#[test]
fn test_duplicate_packet_detection() {
    let detector = DuplicateDetector::new(1000);
    
    // First packet should be unique
    let result1 = detector.detect_duplicate(12345, 67890, 1, None).unwrap();
    assert_eq!(result1, DuplicateDetectionResult::Unique);

    // Second identical packet should be duplicate
    let result2 = detector.detect_duplicate(12345, 67890, 1, None).unwrap();
    assert_eq!(result2, DuplicateDetectionResult::Duplicate);
}

#[test]
fn test_legitimate_reorder_detection() {
    let detector = DuplicateDetector::new(1000);
    
    // Add packet with sequence 100
    detector.detect_duplicate(12345, 67890, 100, None).unwrap();
    
    // Packet with sequence 95 should be legitimate reorder
    let result = detector.detect_duplicate(12346, 67890, 95, None).unwrap();
    assert_eq!(result, DuplicateDetectionResult::LegitimateReorder);
}

#[test]
fn test_sequence_validation() {
    let detector = DuplicateDetector::new(1000);
    
    // Sequence within window should be valid
    assert!(detector.validate_sequence_order(12345, 105, 100).unwrap());
    
    // Sequence far outside window should be invalid
    assert!(!detector.validate_sequence_order(12345, 300, 100).unwrap());
}

#[test]
fn test_different_sessions() {
    let detector = DuplicateDetector::new(1000);
    
    // Same timestamp and sequence, different sessions should both be unique
    let result1 = detector.detect_duplicate(12345, 67890, 1, None).unwrap();
    assert_eq!(result1, DuplicateDetectionResult::Unique);

    let result2 = detector.detect_duplicate(12345, 11111, 1, None).unwrap();
    assert_eq!(result2, DuplicateDetectionResult::Unique);
}

#[test]
fn test_source_ip_correlation() {
    let detector = DuplicateDetector::new(1000);
    let ip1 = IpAddr::from([192, 168, 1, 1]);
    let ip2 = IpAddr::from([192, 168, 1, 2]);
    
    // First packet from IP1
    let result1 = detector.detect_duplicate(12345, 67890, 1, Some(ip1)).unwrap();
    assert_eq!(result1, DuplicateDetectionResult::Unique);

    // Same packet from IP2 should still be detected as duplicate
    let result2 = detector.detect_duplicate(12345, 67890, 1, Some(ip2)).unwrap();
    assert_eq!(result2, DuplicateDetectionResult::Duplicate);
}

#[test]
fn test_cache_eviction() {
    let detector = DuplicateDetector::new(5); // Small cache size
    
    // Fill cache beyond capacity
    for i in 0..10 {
        detector.detect_duplicate(i, 67890, i as u32, None).unwrap();
    }

    let info = detector.get_cache_info();
    assert_eq!(info.current_size, 5); // Should be limited to max size
    assert_eq!(info.max_size, 5);
}

#[test]
fn test_cache_cleanup() {
    let detector = DuplicateDetector::new(1000);
    
    // Add some entries
    for i in 0..10 {
        detector.detect_duplicate(i, 12345, i as u32, None).unwrap();
    }

    let info_before = detector.get_cache_info();
    assert_eq!(info_before.current_size, 10);

    // Cleanup with very short max age should remove all entries
    let removed = detector.cleanup_expired_entries(Duration::from_nanos(1)).unwrap();
    assert_eq!(removed, 10);

    let info_after = detector.get_cache_info();
    assert_eq!(info_after.current_size, 0);
}

#[test]
fn test_statistics_tracking() {
    let detector = DuplicateDetector::new(1000);
    
    // Process some packets
    detector.detect_duplicate(1, 1, 1, None).unwrap(); // Unique
    detector.detect_duplicate(1, 1, 1, None).unwrap(); // Duplicate
    detector.detect_duplicate(2, 1, 2, None).unwrap(); // Unique

    let stats = detector.get_stats();
    assert_eq!(stats.total_packets, 3);
    assert_eq!(stats.duplicates_detected, 1);
    assert_eq!(stats.cache_hits, 1);
    assert_eq!(stats.cache_misses, 2);
}

#[test]
fn test_key_validation() {
    let detector = DuplicateDetector::new(1000);
    
    let valid_key = buckwild_common::protocol::CompositeKey {
        timestamp: 12345,
        session_id: 67890,
        sequence_number: 1,
    };
    assert!(detector.validate_key_components(&valid_key));

    let invalid_key = buckwild_common::protocol::CompositeKey {
        timestamp: 0,
        session_id: 0,
        sequence_number: u32::MAX,
    };
    assert!(!detector.validate_key_components(&invalid_key));
}

#[test]
fn test_reorder_window_boundaries() {
    let detector = DuplicateDetector::new(1000);
    
    // Add packet at sequence 100
    detector.detect_duplicate(12345, 67890, 100, None).unwrap();
    
    // Test boundary conditions for reorder window
    let result_within = detector.detect_duplicate(12346, 67890, 50, None).unwrap(); // Within window
    assert_eq!(result_within, DuplicateDetectionResult::LegitimateReorder);
    
    let result_outside = detector.detect_duplicate(12347, 67890, 1, None).unwrap(); // Outside window
    assert_eq!(result_outside, DuplicateDetectionResult::Unique);
}

#[test]
fn test_concurrent_access_simulation() {
    use std::sync::Arc;
    use std::thread;
    
    let detector = Arc::new(DuplicateDetector::new(10000));
    let mut handles = vec![];
    
    // Simulate concurrent access from multiple threads
    for thread_id in 0..10 {
        let detector_clone = Arc::clone(&detector);
        let handle = thread::spawn(move || {
            for i in 0..100 {
                let timestamp = (thread_id * 1000 + i) as u64;
                let session_id = thread_id as u64;
                let sequence = i as u32;
                
                detector_clone.detect_duplicate(timestamp, session_id, sequence, None).unwrap();
            }
        });
        handles.push(handle);
    }
    
    // Wait for all threads to complete
    for handle in handles {
        handle.join().unwrap();
    }
    
    let stats = detector.get_stats();
    assert_eq!(stats.total_packets, 1000); // 10 threads * 100 packets each
}

#[test]
fn test_reset_statistics() {
    let detector = DuplicateDetector::new(1000);
    
    // Generate some activity
    for i in 0..5 {
        detector.detect_duplicate(i, 12345, i as u32, None).unwrap();
    }
    
    let stats_before = detector.get_stats();
    assert!(stats_before.total_packets > 0);
    
    // Reset statistics
    detector.reset_stats();
    
    let stats_after = detector.get_stats();
    assert_eq!(stats_after.total_packets, 0);
    assert_eq!(stats_after.duplicates_detected, 0);
    assert_eq!(stats_after.cache_hits, 0);
    assert_eq!(stats_after.cache_misses, 0);
}