...
//!
//! These tests are Linux-only as they depend on eBPF ring buffer infrastructure.
...
mod ring_buffer_mock_tests {
    ...
    use buckwild_ebpf::events::{RingBufferConfig, RingBufferManager};
    ...
    async fn test_backpressure_configuration() {
        // Test that backpressure can be configured
        ...
        // Send events to test basic flow
        let total_events = 20;
        let mut results = vec![];

        for i in 0..total_events {
            let buffer =
                create_test_event_buffer(i as u64, i as u64, 1000000, 1500, 0x01, 0x00, 0xC0A80164);
        ...
        let failed = results.iter().filter(|&&r| r != 0).count();

        // Verify backpressure triggered
        assert_eq!(successful + failed, total_events);
        assert_eq!(
            successful, 10,
            "First 10 should succeed (backpressure limit)"
        );
        assert_eq!(failed, 10, "Last 10 should fail due to backpressure");

        // Check statistics
        let stats = manager.get_stats();

        // events_processed counts successfully parsed events (all 20)
        assert_eq!(stats.events_processed, 20);
        // events_dropped counts events that hit backpressure (10)
        assert_eq!(stats.events_dropped, 10);
        // in_flight counts events in channel (10)
        assert_eq!(stats.events_in_flight, 10);
        // bytes_processed counts all parsed bytes
        assert_eq!(stats.bytes_processed, 640); // 20 * 32
    }
    ...
    async fn test_backpressure_recovery() {
        ...
        // Consume events to release backpressure
        {
            let receiver = manager.event_receiver();
            for _ in 0..5 {
                if timeout(Duration::from_millis(100), receiver.recv())
                    .await
                    .is_ok()
                {
                    // Event received
        ...
        // Release permits after consuming events
        for _ in 0..5 {
        ...
        assert_eq!(
            result, 0,
            "Event should succeed after backpressure released"
        );

        let stats = manager.get_stats();
        // Should have processed: 5 initial + 1 after dropped + 1 after release
        assert!(
            stats.events_processed >= 5,
            "Should have processed at least initial events"
        );
        assert!(
            stats.events_dropped >= 1,
            "Should have dropped at least one during backpressure"
        );
    }
    ...
    async fn test_oversized_buffer_accepted() {
        ...
        assert_eq!(
            result, 0,
            "Oversized buffer should be accepted (uses first 32 bytes)"
        );
    ...
    async fn test_mixed_valid_and_invalid_events() {
        ...
        let events = vec![
            (
                create_test_event_buffer(1, 1, 1000000, 1500, 0x01, 0x00, 0xC0A80164),
                true,
            ),
            (vec![0u8; 10], false), // Too small
            (
                create_test_event_buffer(2, 2, 1000000, 1500, 0x01, 0x00, 0xC0A80164),
                true,
            ),
            (vec![0u8; 20], false), // Still too small
            (
                create_test_event_buffer(3, 3, 1000000, 1500, 0x01, 0x00, 0xC0A80164),
                true,
            ),
        ];
