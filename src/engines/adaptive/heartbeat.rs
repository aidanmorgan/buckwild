...
impl HeartbeatState {
    ...
    pub fn should_send_heartbeat(&self) -> bool {
        let guard = self.last_send_time.lock().unwrap_or_else(|e| e.into_inner());
        ...
        match *guard {
            ...
            Some(last_send) => {
                ...
                let jitter = (rand::random::<u64>() % (jitter_ms * 2))
    ...
    pub fn record_send(&self) -> HeartbeatSequence {
        ...
        *self.last_send_time.lock().unwrap_or_else(|e| e.into_inner()) = Some(Instant::now());
    ...
    pub fn record_response(&self, sequence: HeartbeatSequence, sent_time: Instant) {
        ...
        *self.last_receive_time.lock().unwrap_or_else(|e| e.into_inner()) = Some(Instant::now());
    ...
    pub fn record_data_packet(&self) {
        if self.config.enable_suppression {
            *self.last_data_packet_time.lock().unwrap_or_else(|e| e.into_inner()) = Some(Instant::now());
    ...
    pub fn check_timeout(&self) -> Result<(), HeartbeatError> {
        ...
        let last_activity = {
            let send_guard = self.last_send_time.lock().unwrap_or_else(|e| e.into_inner());
            let recv_guard = self.last_receive_time.lock().unwrap_or_else(|e| e.into_inner());
            let data_guard = self.last_data_packet_time.lock().unwrap_or_else(|e| e.into_inner());
    ...
    pub fn reset(&self) {
        ...
        *self.last_send_time.lock().unwrap_or_else(|e| e.into_inner()) = None;
        *self.last_receive_time.lock().unwrap_or_else(|e| e.into_inner()) = None;
        *self.last_data_packet_time.lock().unwrap_or_else(|e| e.into_inner()) = None;
...
impl HeartbeatEngine {
    ...
    pub fn with_defaults(session_id: SessionId) -> Result<Self, HeartbeatError> {
        Self::new(session_id, HeartbeatConfig::default())
    }
    ...
    pub fn generate_heartbeat(&self) -> Result<(HeartbeatSequence, RoundTripTime), HeartbeatError> {
        let _span = span!(Level::INFO, "heartbeat_generate", session_id = %self.session_id).entered();
    ...
    pub fn check_keepalive(&self) -> Result<(), HeartbeatError> {
        let _span = span!(Level::DEBUG, "heartbeat_keepalive", session_id = %self.session_id).entered();
...
mod tests {
    ...
    fn test_heartbeat_timeout_detection() {
        ...
        config.timeout_ms = 100; // 100ms for testing
        let state = HeartbeatState::new(config);
